...
document.write(`O resultado da soma de ${valueA} e ${valueB} é ${sum}<br><br>`)
...
document.write(`O valor total da compra foi R$${total.toFixed(2).toString().replace('.', ',')} <br>`);
document.write(`Forma de pagamento: ${installments}x de R$${installmentValue.toFixed(2).toString().replace('.', ',')} <br><br>`);
...
document.write(`${totalMinutes} minutos equivale à ${totalSeconds} segundos! <br><br>`);
...
const average = ( nota_1 + nota_2 + nota_3 ) / 3;

document.write(`O aluno ${aluno} ficou com média ${average.toFixed(1).toString().replace('.', ',')} <br><br>`);
...
document.write(`Valor variável A: ${a}; valor variável ${b} <br><br>`);
...
const validVotes = 100;
const nullVotes  = 25;
const blankVotes = 10;

const totalVotes = validVotes + nullVotes + blankVotes;
const validPercent = ( validVotes / totalVotes)  * 100;
const nullPercent = ( nullVotes / totalVotes )   * 100;
const blankPercent = ( blankVotes / totalVotes ) * 100;

document.write(`Percentual de votos válidos: ${validPercent.toFixed(2)}%<br>`);
document.write(`Percentual de votos nulos: ${nullPercent.toFixed(2)}%<br>`);
document.write(`Percentual de votos em branco: ${blankPercent.toFixed(2)}%<br><br>`);
...
document.write(`Número de maçãs compradas: ${quantity}. Valor total da compra: ${quantity * price}<br><br>`);
...
document.write(`Nome: ${ name }, Idade: ${ actualYear - birthYear } anos, nasceu em ${birthYear}<br><br>`);
...
if ( number % 2 === 0 ) {
    message = `O número ${number} é par!`;
} else {
    message = `O número ${number} é impar!`;
}
...
const personBirthYear = 2004;
const year = new Date().getFullYear();

if ( (year - personBirthYear) < 16 ) {
